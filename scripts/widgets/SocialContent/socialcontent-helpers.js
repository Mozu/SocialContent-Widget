define(['modules/jquery-mozu', 'underscore', 'hyprlivecontext'],
    function ($, _, HyprLiveContext) {
        ...
        var mobileURLByLink = function(link, mzProtocol) {
            var URL = "";
            if(isMobileDevice()) {
                if(typeof(mzProtocol) === 'undefined' || mzProtocol === "") {
                    mzProtocol = 'bf:'
                }
                switch (link.linkType) {
                    case 'categoryId' :
                        URL = mzProtocol + '//mozu.com/categories/' + link.link;
                        break;
                    case 'productId' :
                        URL = mzProtocol + '//mozu.com/products/' + link.link;
                        break;
                    default :
                        URL = link.link;
                        break;
        ...
        var desktopURLByLink = function(link) {
            return defaultBroswer(link);
        };

        var defaultBroswer = function(link) {
            var URL = "";
            switch (link.linkType) {
                case 'categoryId' :
        ...
        return {
            mobileURLByLink: function (link, mzProtocol) {
                return mobileURLByLink(link, mzProtocol);
            },
